...
class Node:
    ...
    #função Upper Confidence bound (UCT)
    def UCT(self):
        if self.N == 0:
...
class MCTS:
    ...
    def selection(self):
        ...
        while len(node.filhos) != 0:
            ...
            maior_uct = max(filhos, key=lambda n: n.UCT()).UCT()
            max_nodes = [n for n in filhos if n.UCT() == maior_uct]
    ...
    def expand(self, pai: Node, mancala) -> bool:
        ...
        filhos = [Node(move, pai) for move in mancala.movimentos_possivies(self.jog)]
    ...
    def simulation(self, mancala):
        while mancala.fim_jogo() == -1 and mancala.jogada_impossivel(self.jog) == False:
            jogadas_possiveis = mancala.movimentos_possivies(self.jog)
    ...
    def best_move(self):
        ...
        maior_uct = max(self.raiz.filhos.values(), key=lambda n: n.N).N
        max_nodes = [n for n in self.raiz.filhos.values() if n.N == maior_uct]
    ...
    def move(self, move):
        if move in self.raiz.filhos:
            posicao = move
            mancala.move(self.jog, posicao)
            self.raiz = self.raiz.filhos[move]
            return
